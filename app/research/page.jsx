          ...
          <div className="space-y-8">
            {[
              ...
              },
              {
                ...
                description:
                  'This research develops a multi-layer quantitative risk engine engineered for environments where survival is a mathematical requirement, not a philosophical ideal. The framework integrates regime-aware volatility modeling, Hidden Markov Models for state transitions, Monte Carlo stress cones, and tail-amplification diagnostics to construct a dynamic risk envelope around every strategy. Position sizing is governed by volatility-normalized allocation rules and adaptive capital floors that ratchet upward with performance, enforcing drawdown limits without constraining upside drift. Predictive risk measures—such as forward-looking VaR, Expected Shortfall, and scenario-conditioned drawdown distributions—are generated through stochastic path simulations that incorporate structural breaks, liquidity contractions, and correlation shocks. Execution risk is modeled through slippage surfaces, order-book impact estimators, and asymmetric fill-probability curves, ensuring that theoretical performance aligns with real-market constraints. Continuous monitoring of distributional drift, volatility clustering, and liquidity microfractures allows the system to compress exposure proactively during hostile regimes and expand it when conditions normalize. The result is a mathematically disciplined risk architecture where every position, every interval, and every execution decision is evaluated against a probabilistic shield built to institutional standards of transparency, robustness, and capital preservation.',
              ...
              },
              {
                ...
                description:
                  'This research develops a production-grade trading architecture engineered with the discipline of an execution war room—an orchestrated network of autonomous components operating under strict time, risk, and reliability constraints. Market data enters through a zero-latency intake layer, where each packet is validated and transformed into a feature engine that runs the identical computational graph used in research, guaranteeing feature parity between backtesting and live execution. Distributed services handle feature computation, signal evaluation, and routing independently yet remain synchronized through shared schemas and coordinated clocks. Signal evaluators fire in deterministic cycles, producing confluence vectors and structural readouts that flow into a hardened execution router capable of dynamic rerouting in response to slippage, volatility spikes, or microstructure fractures. The infrastructure relies on containerized services, versioned models, rolling deployments, and fault-tolerant workers, while every subsystem emits granular telemetry—latency, drift, fill probability, and state transitions—into real-time dashboards. The architecture behaves as a living machine: redundant, observable, and auditable at every step, ensuring that execution never deviates from the strategy’s core logic and that reliability is guaranteed by engineering, not intuition.',
              ...
              const isFeatured =
                ...
                paper.title === 'Live Trading System Architecture';
